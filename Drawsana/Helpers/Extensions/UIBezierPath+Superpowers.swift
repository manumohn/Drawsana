...
private enum LengthCalculationPrecision: Int {
    case low = 50
    case normal = 100
    case high = 150
}

private let lengthCalculationPrecision: LengthCalculationPrecision = .normal

/// The precision with which to calculate the perpendicular points and distances.
/// Higher precision is naturally more expensive to compute.
private enum PerpendicularCalculationPrecision: CGFloat {
    case low = 15
    case normal = 5
    case high = 2
}

private let perpendicularCalculationPrecision: PerpendicularCalculationPrecision = .normal
//  -
...
private extension CGPath {
    func apply(closure: @escaping CGPathApplierClosure) {
        apply(info: unsafeBitCast(closure, to: UnsafeMutableRawPointer.self)) { info, element in
...
private extension UIBezierPath {
    func extractPathElements() -> [BezierPathElement] {
        if let pathElements = mx_pathElements {
            return pathElements
        }
        ...
        cgPath.apply { element in
            ...
            if type == .closeSubpath {
                let startPoint = pathElements.last?.endPoint ?? .zero
                let endPoint = pathElements.first?.startPoint ?? .zero
                let pathElement = BezierPathElement(type: type, startPoint: startPoint, endPoint: endPoint, controlPoints: [])
                pathElements.append(pathElement)
                currentPoint = endPoint
            }
        }
...
private extension CGPoint {
    ...
    func linearSlope(to: CGPoint, t _: CGFloat) -> CGFloat {
        let dx = to.x - x
        let dy = to.y - y
    ...
    func linearTangentAngle(to: CGPoint, t _: CGFloat) -> CGFloat {
        let dx = to.x - x
        let dy = to.y - y
    ...
    func quadCurveLength(to: CGPoint, controlPoint c: CGPoint) -> CGFloat {
        ...
        for idx in 0 ..< iterations {
            ...
            let p = quadBezierPoint(to: to, controlPoint: c, t: t)
            let pp = quadBezierPoint(to: to, controlPoint: c, t: tt)
    ...
    func quadSlope(to: CGPoint, controlPoint: CGPoint, t: CGFloat) -> CGFloat {
        let dx = _quadSlope(t, x, controlPoint.x, to.x)
        let dy = _quadSlope(t, y, controlPoint.y, to.y)
    ...
    func cubicCurveLength(to: CGPoint, controlPoint1 c1: CGPoint, controlPoint2 c2: CGPoint) -> CGFloat {
        ...
        for idx in 0 ..< iterations {
            ...
            let p = cubicBezierPoint(to: to, controlPoint1: c1, controlPoint2: c2, t: t)
            let pp = cubicBezierPoint(to: to, controlPoint1: c1, controlPoint2: c2, t: tt)
    ...
    func cubicSlope(to: CGPoint, controlPoint1 c1: CGPoint, controlPoint2 c2: CGPoint, t: CGFloat) -> CGFloat {
        let dx = _cubicSlope(t, x, c1.x, c2.x, to.x)
        let dy = _cubicSlope(t, y, c1.y, c2.y, to.y)
